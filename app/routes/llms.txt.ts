...
export async function loader() {
  ...
  const lines = [
    ...
    `- Developer / API: https://docs.hedgeapp.trade/developers`,
    "",
    "## Native pool",
    "",
    "USDG parimutuel on Robinhood Chain memes. Odds are the pools. 1x. No venue CLOB.",
    `- Desk: ${origin}/pool`,
    `- Markets: ${origin}/api/native`,
    "",
    "## 1x spot (external apps)",
    "",
    "Another product lists and quotes the live 1x book, then sends the user to a prefilled ticket. The fill stays in Hedge. No builder key.",
    "",
    `- Capability card: ${origin}/api/spot`,
    `- Markets: ${origin}/api/spot/markets`,
    `- Quote: ${origin}/api/spot/quote?marketId=<id>&side=yes&amount=5`,
    `- Ticket: ${origin}/api/spot/ticket?marketId=<id>&side=yes&amount=5`,
    `- Ticket URL: ${origin}/market/{eventSlug}?m={marketId}&s=yes&amt=5`,
    `- Docs: https://docs.hedgeapp.trade/guides/spot`,
    "",
    "## Agent Wall",
    "",
